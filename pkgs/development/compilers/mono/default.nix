...
stdenv.mkDerivation rec {
  ...
  configureFlags = "--x-includes=${libX11.dev}/include --x-libraries=${libX11.dev}/lib --with-libgdiplus=${libgdiplus}/lib/libgdiplus.so ${llvmOpts}";
  ...
  postBuild = ''
    find . -name 'config' -type f | while read i; do
        sed -i "s@libX11.so.6@${libX11.dev}/lib/libX11.so.6@g" $i
