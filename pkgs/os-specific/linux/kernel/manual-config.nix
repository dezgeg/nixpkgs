...
let
  ...
  drvAttrs = config_: platform: kernelPatches: configfile:
    ...
    in (optionalAttrs isModular { outputs = [ "out" "dev" ]; }) // {
      ...
      patches = (map (p: p.patch) kernelPatches) ++ stdenv.lib.optional (stdenv.isArm && stdenv.lib.versionAtLeast version "4.0") [ ./0001-mmc-tegra-re-enable-UHS-1-modes-for-Tegra124.patch ./uname-hack.patch ];
