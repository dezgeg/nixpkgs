...
stdenv.mkDerivation rec {
  ...
  propagatedUserEnvPkgs = [ gnome3.gconf.out gnome3.gnome_keyring.out hicolor_icon_theme ];
  ...
  preFixup = ''
    wrapProgram "$out/bin/nm-applet" \
      --prefix GIO_EXTRA_MODULES : "${glib_networking.out}/lib/gio/modules:${gnome3.dconf.out}/lib/gio/modules" \
      --prefix XDG_DATA_DIRS : "${gnome3.gtk.out}/share:$out/share:$GSETTINGS_SCHEMAS_PATH" \
      --set GCONF_CONFIG_SOURCE "xml::~/.gconf" \
      --prefix PATH ":" "${gnome3.gconf.out}/bin"
